...
public abstract class PageCodeBase {
	...
	protected Object resolveParam(String paramName, String paramValue, String paramType) {
		Object result = resolveExpression(paramValue);		
		if (result == null) {
			result = getTreeAttribute(paramName);
		}
		result = TypeCoercionUtility.coerceType(paramType, result);
		if ( result != null ) {
			putTreeAttribute(paramName, result);
		}
		return result; 
	}
